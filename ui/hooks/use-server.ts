...
export async function UseServer({
    ...
    const redirectUrl = path ? `${authUrl}?ref=${encodeURIComponent(appUrl)}&path=${encodeURIComponent(path)}` : `${authUrl}?ref=${encodeURIComponent(appUrl)}`
...
const refresh = async (rt: string): Promise<{ accessToken: string, refreshToken: string } | false> => {
...
const dataFetch = async ({ endpoint, at, method, body }: {
    endpoint: string,
    method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE";
    at: string;
    body?: {}
