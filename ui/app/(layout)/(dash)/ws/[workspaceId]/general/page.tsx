...
export default function WorkspaceGeneral() {
    ...
    useEffect(() => {
        if (selectedWorkspace) {
            ...
            setStatus(selectedWorkspace.status)
        }
    }, [selectedWorkspace])
    ...
    const { mutateAsync, isPending, error, isError: isUpdateError } = updateWorkspace

    const handleWorkspaceUpdate = async (e: React.FormEvent<HTMLFormElement>, forDelete?: boolean) => {
        e.preventDefault();
        if (forDelete) {
            await mutateAsync({
            ...
            const hasNameChanged = name && name.trim().length >= 2 && name !== selectedWorkspace.name;
    ...
    return (
        <div className="w-full flex flex-col gap-6 items-start justify-start max-w-[600px] sm:py-8">
            <Header title='General' icon={<Settings />} type='workspace setting' />
            <form onSubmit={handleWorkspaceUpdate} className='w-full flex flex-col gap-8'>
                <FieldGroup>
                    <FieldSet className='flex flex-col gap-8'>
                        <FieldLegend>General Workspace Information</FieldLegend>
                        <FieldDescription>
                            Edit your workspace's basic settings, such as its name or status. You can also delete the workspace if it's no longer needed.
                        </FieldDescription>
                        <FieldGroup className='max-w-100'>
                            <Field>
                                ...
                                <Input
                                    ...
                                    id="create-workspace"
                                    placeholder={selectedWorkspace.name ? selectedWorkspace.name : "Enter New workspace name..."}
                                />
                            </Field>
                            <Field>
                                ...
                                >
                                    <SelectTrigger disabled={isPending || isFetching}>
                                        <SelectValue
                                            placeholder={selectedWorkspace.status ? selectedWorkspace.status : "Select New workspace Status"}
                                        />
                                    </SelectTrigger>
                                    <SelectContent>
                ...
                <FieldGroup>
                    <FieldSet className='flex flex-col gap-8'>
                        <FieldLegend className='text-red-500'>Deactivate Workspace</FieldLegend>
                        <FieldDescription>
                            This action will deactivate your workspace. All of your data will be preserved, and you can reactivate the workspace later if needed from trash section.
                        </FieldDescription>
                    </FieldSet>
                    <AlertDialog>
