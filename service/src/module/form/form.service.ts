...
export class FormService {
    ...
    constructor(
        private readonly formcoreService: FormcoreService
    ) { }

    async createForm(userId: string, workspaceId: string, data: CreateFormDto): Promise<ResponseDataType> {
        const form = await this.formcoreService.createForm(userId, workspaceId, data)
        if (!form) {
    ...
    async updateForm(userId: string, formId: string, data: UpdateFormDto): Promise<ResponseDataType> {
        const form = await this.formcoreService.updateForm(userId, formId, data)
        if (!form) {
            throw new NotFoundException(
                "form not found or failed to update"
            )
        }
        return {
    ...
    async deleteForm(userId: string, formId: string): Promise<ResponseDataType> {
        ...
        if (!form) {
            throw new NotFoundException(
                'form not found or failed to delete'
            )
        }
        return {
    ...
    async getTrashForm(userId: string, workspaceId: string): Promise<ResponseDataType> {
    ...
    async getForms(userId: string, workspaceId: string): Promise<ResponseDataType> {
